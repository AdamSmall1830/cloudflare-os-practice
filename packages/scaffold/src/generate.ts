...
- Regulatory mapping: see COMPLIANCE.md (each control → the requirement it satisfies; review with counsel).
...
export function generateFiles(c: ClientRecord): GeneratedFile[] {
  ...
  for (const s of custom) {
    const approver = PAYMENT_SYSTEMS.has(s.id)
  ...
  // Skills seed the SAME pilot set as evals and workflow specs (designModel.pilots:
  // score-ranked, C-risk excluded) — so a workflow spec never references a skill
  // file that wasn't emitted, and a C-risk use case never gets a skill seed.
  for (const u of designModel(c).pilots) {
    files.push({
  ...
  for (const w of workflowSpecs(c)) {
    files.push({ path: w.file, content: w.markdown });
  }
  ...
  files.push({ path: "evals/promptfooconfig.yaml", content: promptfooConfig(c) });
  ...
  files.push({ path: "COMPLIANCE.md", content: complianceMarkdown(c) });
...
- \`workflows/\` — pattern-instantiated specs for every scheduled/event pilot (patterns: the practice repo's docs/workflow-patterns.md); complete with the champion before implementing
- \`evals/\` + \`EVALS.md\` — the evaluation suites (golden + red-team) and run protocol; pre-pilot gate and release-bump regression net. \`evals/promptfooconfig.yaml\` automates the model-facing subset via \`npx promptfoo eval\` (point it at your dynamic route)
...
- \`COMPLIANCE.md\` — per-vertical mapping of each control to the regulatory requirement it satisfies (HIPAA / SEC / bar rules / privacy), with human sign-offs; a mapping aid to review with counsel, not legal advice
...
`,
  ...
  // Guard against two use-case names slugging to the same filename (would
  // silently overwrite). Disambiguate later duplicates with -2, -3, … so every
  // skill/eval/workflow file survives.
  const usedPaths = new Set<string>();
  for (const f of files) {
    if (!usedPaths.has(f.path)) {
      usedPaths.add(f.path);
      continue;
    }
    const dot = f.path.lastIndexOf(".");
    const stem = dot > 0 ? f.path.slice(0, dot) : f.path;
