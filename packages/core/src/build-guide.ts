...
7. **Per-user attribution is automatic (identity-aware controls).** Because the gateway sits behind Access, it stamps each request with the signed-in user as \`cf.user_id\` — no client-side plumbing. Use it for **per-user spend limits**, **per-user log/analytics filtering**, and controlling who may call the gateway. This makes the METRICS spend-per-user line native, and complements the Dynamic Route's per-team budget nodes.`,
...
4. Policy check against the rule of thumb: if this system needs your own approval semantics on writes, ethical walls, or minimum-necessary scoping, take it off the portal route and build the gatekeeper instead.`,
  ...
  // AI Search is needed for any source that routes to R2 + AI Search — i.e.
  // anything NOT explicitly a wiki/templates/live-data route (mirror of the
  // routing default below), so an out-of-schema type still gets its step.
  const needsAiSearch = kn.some((k) => k.type !== "wiki" && k.type !== "templates" && k.type !== "data");
  if (needsAiSearch) {
    steps.push({
      id: "aisearch",
...
3. Upload a small starter batch of SOP documents and let it index; set the re-index behavior for new uploads. (AI Search can also crawl a site without a full sitemap if the client's knowledge lives on an internal web page.)
...
export function deploymentJsonc(c: ClientRecord): string {
  ...
  const stag = stagingFor(c);
  const sl = slug(c.name);
  // JSON-encode every interpolated value so a stray quote in a record field
  // can't inject extra JSONC keys or produce invalid config.
  const q = (v: string): string => JSON.stringify(v);
  const admins = (c.adminEmails || "you@yourfirm.com").split(",").map((e) => q(e.trim()));
  const accessLine =
    c.idp === "access"
      ? `"accessAudience": ${q(ph(c.audience, "<PASTE AUD TAG — Access step>"))},`
      : `// no Access audience — sign-in via ${c.idp === "google" ? "AUTH_GATEKEEPERS=google" : "password auth"}`;

  return `{
  // ---- ${c.name} · generated by @cfos-practice/core ----
  "accountId": ${q(ph(c.accountId, "<PASTE ACCOUNT ID>"))},
  "hostname": ${q(host)},
  // staging deployment (second copy of this file or env-specific block):
  //   "hostname": ${q(stag)},
  "workerName": ${q(sl + "-os")},
  ${accessLine}
  "adminEmails": [${admins.join(", ")}]
